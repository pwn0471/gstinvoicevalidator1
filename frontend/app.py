...
def render_confusion_matrix(report):
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="coolwarm", xticklabels=labels, yticklabels=labels, ax=ax)
...
def main():
    ...
    with st.sidebar:
        st.header("🔍 About")
        st.write("This app uses a Machine Learning model to classify GST invoices as Valid or Suspicious based on amount, GST rate, total, and vendor frequency.")
    ...
    # Main content
    st.markdown('<h1 class="main-header">📋 GST Invoice Validator</h1>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">AI-Powered Invoice Classification for GST Compliance</p>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        with st.form(key="invoice_form"):
            invoice_amount = st.number_input(
                ...
                step=50.0,
                help="The base amount before GST"
            )
            gst_rate = st.selectbox(
                "GST Rate (%)",
                [0.0, 5.0, 12.0, 18.0, 28.0],
                index=[0.0, 5.0, 12.0, 18.0, 28.0].index(st.session_state.get('gst_rate', 18.0)),
                help="Applicable GST percentage"
            )
            total_amount = st.number_input(
                ...
                step=10.0,
                help="Total amount including GST"
            )
            vendor_frequency = st.slider(
                ...
                help="Number of invoices from this vendor"
            )
            
            use_backend_api = st.checkbox("Use Backend API", value=False, help="Send request to FastAPI server")
            submitted = st.form_submit_button("🔍 Validate Invoice", use_container_width=True)
    ...
    with col2:
        ...
        if submitted:
            ...
            with st.spinner("Analyzing invoice..."):
                try:
                    if use_backend_api:
                        api_endpoint = st.text_input("API Endpoint", value="http://127.0.0.1:8000/predict", key="api_endpoint")
                        result = predict_api(api_endpoint, payload)
                        ...
                        prediction, confidence = predict_local(
                            model, invoice_amount, gst_rate, total_amount, vendor_frequency
                        )

                    # Display result
                    result_class = "valid" if prediction == "Valid" else "suspicious"
                    st.markdown(f"""
                    <div class="prediction-card">
                        <h3>Prediction: <span class="{result_class}">{prediction}</span></h3>
                        <p><strong>Confidence:</strong> {confidence:.1%}</p>
                        <p><strong>Invoice Amount:</strong> ₹{invoice_amount:,.0f}</p>
                        <p><strong>GST Rate:</strong> {gst_rate}%</p>
                        <p><strong>Total Amount:</strong> ₹{total_amount:,.0f}</p>
                        <p><strong>Vendor Frequency:</strong> {vendor_frequency}</p>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    if st.session_state.get('show_performance', False) and report:
        st.markdown("---")
        ...
        with st.expander("📋 Detailed Classification Report"):
            st.dataframe(pd.DataFrame(report["classification_report"]).transpose())
